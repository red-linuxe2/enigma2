...
void eWidgetAnimation::nexttick(int inc)
{
	if (!m_active)
		return;
	...
	if (m_move_length)
	{
		if (m_move_current_nexttick >= m_move_length)
		{
			m_active = 0;
		...
		#if 0
		xdiff *= m_move_current_nexttick;
		xdiff /= m_move_length;

		ydiff *= m_move_current_nexttick;
		ydiff /= m_move_length;
		#endif

		ePoint res(m_move_end.x() + xdiff, m_move_end.y() + ydiff);

		m_move_current_nexttick += inc;
